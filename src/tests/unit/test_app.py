...
class NoclistRetrieverTests(TestCase):
    ...
    @patch("app.sys", wraps=sys)
    def test__main_exception_exits_with_code_1(self, spy):
        spy.exit.return_value = None
        self.noclist_client_mock.get_authentication_token.side_effect = Exception
        self.noclist_retriver_app.main()

        spy.exit.assert_called_once_with(1)
